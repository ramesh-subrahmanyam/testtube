...
def stats(pnl_series, positions=None, exposure_days=None):
    """
    ...
    Args:
        ...
        exposure_days (pd.Series, optional): Boolean series indicating which days have market exposure.
                                            If provided, this is used instead of lagged positions for
                                            calculating exposure-based metrics.
    ...
    Example:
        ...
        performance = stats(pnl, df['Position'], df['Is_Exposure_Day'])
    ...
    # Use Is_Exposure_Day if provided, otherwise fall back to lagged positions
    if exposure_days is not None:
        # Use the explicit exposure day markers set by the strategy/modifier
        exposure_mask = exposure_days.astype(bool)
        ...
        # FALLBACK: Use lagged positions if Is_Exposure_Day not provided
        # IMPORTANT: PnL is generated by LAGGED positions (position on day T generates PnL on day T+1)
        # This is only correct for "entry at close" strategies
    ...
    # This should use PnL only from exposure days, not total PnL from all days
    if num_exposure_days > 0:
        if exposure_days is not None:
            # Use PnL only from marked exposure days
            pnl_per_exposure_day = pnl_series[exposure_days.astype(bool)].sum() / num_exposure_days
        elif positions is not None:
            # Fallback: use lagged positions
            lagged_positions = positions.shift(1).fillna(0)
            exposure_mask_calc = lagged_positions != 0
            pnl_per_exposure_day = pnl_series[exposure_mask_calc].sum() / num_exposure_days
        else:
...
def extract_trades(df, positions_col='Position_At_Close', price_col='Close', pnl_col='Slipped_PnL', use_open_for_entry=False, use_open_for_exit=False):
    """
